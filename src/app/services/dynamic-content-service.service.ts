...
export class DynamicContentService {
  ...
  public renderSection(data, component = ItemSectionComponent) {
    /*
    const itemHeaderComponentFactory = this.factoryResolver
      .resolveComponentFactory(ItemHeaderComponent);

    const componentRef  = this.rootViewContainer.createComponent(
      itemHeaderComponentFactory);

    (componentRef.instance).data = headerData;

    if ('@graph' in item.json) {
      this.parseGraph(item.json);
    } else {
      this.parseNonGraph(item.json);
    }
